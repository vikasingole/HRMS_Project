...
public class UserServiceImpl implements UserServicei {
    ...
    public User findByUsername(String username) {
        System.out.println("Looking for username: " + username); // Step 1 log

        Optional<User> userOptional = repository.findByUsername(username);

        if (userOptional.isPresent()) {
            System.out.println("User found: " + username); // Step 2 log
            return userOptional.get();
        } else {
            System.out.println("User NOT found in DB: " + username); // Step 3 log
    ...
    public String resetPassword(String email, String otp, String newPassword) {
        ...
        if(userOptional.isPresent()){
            ...
            if(LocalDateTime.now().isAfter(otpExpiryTime)){
                ...
                auditLogService.logInfo(user.getUserId(), Action.PASSWORD_CHANGE, null);
